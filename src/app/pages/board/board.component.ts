...
export class BoardComponent implements AfterViewInit {
    ...
    if (fromClassId && toClassId) {
      ...
      if (fromNode && toNode) {
        const fromPortPos = fromNode
          .findPort('T')
          .getDocumentPoint(go.Spot.Center); // Por ejemplo, usas el puerto 'T' para Top
  ...
  exportDiagram(): void {
    ...
    this.httpClient
      .post(
        'http://localhost:3000/api/board/export',
  ...
  importDiagram(event: any): void {
    ...
    this.httpClient
      .post('http://localhost:3000/api/board/import', formData, {
        headers: {
